...
interface AgentConfig {
  ...
  cloudflare: {
    ...
    /** Custom domain for the Pages project (optional, falls back to pages.dev). */
    customDomain?: string;
  };
}
...
function isAgentConfig(value: unknown): value is AgentConfig {
  ...
  // customDomain is optional, but if present must be a string
  if (cloudflare['customDomain'] !== undefined && typeof cloudflare['customDomain'] !== 'string') return false;
...
async function routeMessage(
  ...
  const normalizedText = text.trim().toLowerCase();
   ...
   // ── 0. Handle pending deploy-choice state (YES/NO only) ─────────────────
   ...
   if (pendingRaw !== null) {
     ...
     if (normalizedText === 'yes') {
       try {
         ...
         const commitMessage =
           `feat: A3lix deploy — ${pending.summary}\n\n` +
         ...
         const liveUrl = config.cloudflare.customDomain
           ...
           : `https://${config.cloudflare.pagesProjectName}.pages.dev`;
         ...
         await sendTelegramMessage({
           ...
           text:
             ...
             `⏱️ Site will update in about 2–5 minutes.`,
           botToken: env.TELEGRAM_BOT_TOKEN,
         }).catch(() => undefined);
       } catch (error: unknown) {
         if (error instanceof GitHubError) {
