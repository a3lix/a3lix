...
A3lix understands the request, edits the actual code in GitHub, and lets the requester choose between a **LIVE** deploy (direct commit to the base branch) or a **PREVIEW** deploy (branch + approval flow).
...
5. **The worker** stores the proposed changes and asks the requester to reply `LIVE` or `PREVIEW`
6. If they choose **LIVE**, A3lix commits directly to the base branch and Cloudflare Pages deploys production
7. If they choose **PREVIEW**, A3lix creates a preview branch and Cloudflare Pages builds a preview deployment
8. **The preview URL** is sent back (format: `https://<deployment-hash>.<project>.pages.dev/`)
9. For previews, `YES` merges to `main`; `NO` discards the pending approval and notifies the requester
...
> ✅ **Preview deployments are on by default.** Every preview branch push automatically generates a Pages URL in the form `https://<deployment-hash>.<project>.pages.dev/`. No extra configuration is needed.
...
| `limits.previewExpiryHours` | `number` | Hours before pending preview approvals expire from KV |
| `limits.requireApprovalForAll` | `boolean` | Reserved config field (not yet hard-enforced in current worker flow) |
...
|------|-----|-----------------|
| **Owner** | The site owner (you) | Approve or reject preview changes, view audit log, access all commands, manage roles via `/addeditor` and `/addviewer` |
| **Editor** | Trusted clients / team members | Request content changes, choose `LIVE` or `PREVIEW`, and approve/reject their own preview submissions |
...
|---------|--------|-------------|
| **v0.1 — MVP** | 🚧 In progress | Telegram webhook, Workers AI, GitHub live + preview deploy modes, Cloudflare Pages previews, KV rate limiting, path guardrails |
